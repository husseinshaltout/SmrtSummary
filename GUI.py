...
# Scan line selection
def scan_line_selection_window():
    ...
    rx = int(cols)/size  # Image size on x(width) / image resize(canvas width)
    ry = int(row)/size  # Image size on y(height) / image resize(canvas height)
    var = tk.IntVar()
    root = tk.Toplevel()
    ...
    slider = tk.Scale(root,
                      from_=0, to=cols, variable=var, orient=tk.HORIZONTAL)
    slider.pack(side=tk.BOTTOM, fill=tk.X)
    button = tk.Button(root, text="Select scanline",
                       command=get_scanline_coordinates)
    button.pack(anchor=tk.CENTER)


def get_scanline_coordinates():
    global selection
...
def Video_window(video_location):
    global Vvar, Vrx, Vry, Vrow, Vcols, Vcanvas, VIDEO_LOCATION
    VIDEO_LOCATION = video_location
    vw = 160  # Image resize and canvas width
    vh = 600  # Image resize and canvas height
    Vrx = int(cols)/vw  # Image size on x(width) / image resize(canvas width)
    Vry = int(row)/vh  # Image size on y(height) / image resize(canvas height)
    if(len(VIDEO_LOCATION) == 0):
        tkinter.messagebox.showinfo(title="Get Summary",
                                    message="Please create video summary "
                                            "to play video")
    else:
        ...
        slider = tk.Scale(root, from_=0, to=Vcols,
                          variable=Vvar, orient=tk.HORIZONTAL)
        slider.pack(side=tk.BOTTOM, fill=tk.X)
        button = tk.Button(root, text="Select line",
                           command=get_video_scanline_coordinates)
        button.pack(anchor=tk.CENTER)
    print(len(VIDEO_LOCATION))


def get_video_scanline_coordinates():
    ...
    print(ts)
    tk.messagebox.showinfo(title="Play video",
                           message="Timestamp: %s \n Press q to exit" % ts)
    print(VIDEO_LOCATION)
    video_play(Vselection, VIDEO_LOCATION)
...
class Toplevel1:
    ...
    def __init__(self, top=None):
        ...
        # bCanvas showing summary image
        self.canvas = tk.Canvas(top)
        self.canvas.place(relx=0.028, rely=0.022,
                          relwidth=0.5, relheight=0.878)
        # Listbox showing all frames
        self.Scrolledlistbox1 = tk.Listbox(top)
        self.Scrolledlistbox1.place(relx=0.563, rely=0.022,
                                    relheight=0.878, relwidth=0.142)
        ...
        self.S_canvas.place(relx=0.720, rely=0.022,
                            relheight=0.250, relwidth=0.250)
        # Load video
        ...
        self.video_location_label = tk.Label(top, text="")
        self.video_location_label.place(relx=0.720, rely=0.36,
                                        height=12, relwidth=0.250)
        # Split Video button
        self.split_video_btn = tk.Button(top, command=self.split_video)
        self.split_video_btn.place(relx=0.803, rely=0.4, height=24, width=85)
        self.split_video_btn.configure(text='''Split video''')
        # Select scanline Button
        self.select_scanline_btn = tk.Button(top, command=lambda: self.isVideo_splitted())
        self.select_scanline_btn.place(relx=0.803, rely=0.5,
                                       height=24, width=85)
        self.select_scanline_btn.configure(text='''Select scanline''')
        # Get Summary Button
        self.get_summary_btn = tk.Button(top, command=self.get_summary_handler)
        self.get_summary_btn.place(relx=0.803, rely=0.6, height=24, width=85)
        self.get_summary_btn.configure(text='''Get summary''')
        # Get video Button
        self.video_btn = tk.Button(top, command=lambda: Video_window(self.video_location_label['text']))
        self.video_btn.place(relx=0.803, rely=0.7, height=24, width=85)
        self.video_btn.configure(text='''Open video''')
    ...
    def split_video(self):
        ...
        locationLabel = self.video_location_label['text']
        if(locationLabel == ""):
            tkinter.messagebox.showinfo(title="No video",
                                        message="Please select a video first")
        else:
            tkinter.messagebox.showinfo(title="Please Wait",
                                        message="Don't close any windows,"
                                                "video is proccessing")
            print(locationLabel)
            self.ss.split(locationLabel)
            self.show_frames_list()
            self.split_flag = 1

    # New scan_line_selection_window callback with conditon
    def isVideo_splitted(self):
        if(self.split_flag == 0):
            tkinter.messagebox.showinfo(title="Split Video",
                                        message="Please split video first")
        else:
            scan_line_selection_window()
            self.scanline_flag = 1

    # Select scanline to crop and get video summary
    def get_summary_handler(self):
        if(self.scanline_flag == 0):
            tkinter.messagebox.showinfo(title="No Scanline",
                                        message="Please select scanline")
        else:
    ...
    # File dialog for browsing to open video
    def fileDialog(self):
        self.filename = tk.filedialog.\
                        askopenfilename(initialdir="/",
                                        title="Select file",
                                        filetypes=(("mp4 files", "*.mp4"),
                                                   ("all files", "*.*")))
        print(self.filename)
        self.video_location_label.configure(text=self.filename)
