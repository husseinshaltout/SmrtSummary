...
class SmrtSummary:
    ...
    def split_to_frames(self) -> None:
        ...
        while(self.cap.isOpened()):
            ...
            if not ret:
                break
            cv.imwrite(os.path.join(self.framesLocation,
    ...
    def crop_at_scanline(self, scanlineX: int) -> None:
        for image in self.framesFileList:
            ...
            cv.imwrite(os.path.join(self.croppedFramesLocation,
                       image.split("\\")[-1]), cropped)

    def concatenate_cropped_frames(self) -> None:
        # Concatenate all cropped images horizontaly
        for i in range(len(self.framesFileList)):
            if(i == 0):
                numpy_horizontal = cv.imread(
                                   os.path.join(self.croppedFramesLocation,
                                                "frame{0}.jpg").format(i))
            else:
                img = cv.imread(os.path.join(self.croppedFramesLocation,
                                             "frame{0}.jpg").format(i))
