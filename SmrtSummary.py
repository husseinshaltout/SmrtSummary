...
class SmrtSummary:
    ...
    def scanline(self, x):
        ...
        for filename in self.fpath:
            ...
            cv.imwrite(os.path.join(self.C_path, filename.split("\\")[-1]),
                       cropped)
    ...
    def mvideo(self):
        img_array = []
        for x in range(len(self.fpath)):
            img = cv.imread(os.path.join(self.C_path,
                                         "frame{0}.jpg").format(x))
            height, width, layers = img.shape
            size = (width, height)
            img_array.append(img)
        out = cv.VideoWriter('project.avi', cv.VideoWriter_fourcc(*'DIVX'), 30,
                             size)
