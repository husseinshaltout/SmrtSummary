...
class RegisterTest(BaseTest):
    ...
    def test_username_already_exist(self) -> None:
        response = self.client.post(self.registerURL, self.user,
                                    format='text/html')
        response = self.client.post(self.registerURL, self.userAlreadyExist,
                                    format='text/html')
    ...
    def test_email_already_exist(self) -> None:
        response = self.client.post(self.registerURL,
                                    self.user, format='text/html')
        response = self.client.post(self.registerURL, self.emailAlreadyExist,
                                    format='text/html')
        messages = list(get_messages(response.wsgi_request))
...
class LoginTest(BaseTest):
    ...
    def test_invalid_login(self) -> None:
        ...
        response = self.client.post(self.loginURL, {'username': '',
                                                    'password': 'password'},
                                    format='text/html')
        messages = list(get_messages(response.wsgi_request))
...
class LogoutTest(BaseTest):
    def test_logout(self) -> None:
        register = self.client.post(self.registerURL, self.user,
                                    format='text/html')
        self.assertEqual(register.status_code, 302)
        response = self.client.post(self.logoutURL, self.user,
                                    format='text/html')
        self.assertEqual(response.status_code, 302)
        messages = list(get_messages(response.wsgi_request))
